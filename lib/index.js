...
function MasonryList({numberOfColumns, data, keyExtractor, contentContainerStyle, ListHeaderComponent, renderItem: Brick, ...rest}) {
				...
				{Boolean(columns.length) && columns.map(({content, id}) => (
					<View key={id}>
						<View onLayout={handleOnLayout(id)}>
							{Boolean(content.length) && content.map(({item}, key) => 
								<Brick item={item} key={keyExtractor(item)} />
							)}
						</View>
					</View>
				))}
